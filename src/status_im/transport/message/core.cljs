...
(fx/defn handle-messages [cofx messages]
  (models.message/receive cofx messages))
...
(fx/defn process-response [cofx response-js]
  (let [chats (.-chats response-js)
    ...
    (cond
      ...
      (seq messages)
      (let [messages-clj (types/js->clj (.splice messages 0 50))]
        (fx/merge cofx
                  {:utils/dispatch-later [{:ms 20 :dispatch [::process response-js]}]}
                  (handle-messages (map data-store.messages/<-rpc messages-clj)))))))
